...
var waited_develop=function(){
    var nexts = document.getElementsByClassName("scroll");
    var cons = document.getElementsByTagName("section");
    var tags = document.getElementsByTagName("aside")[0].children;
    let topName = ["top1", "top2", "top3", "top4"];


    cons[0].style.display = "block";
    tags[0].className = "cur_a";

    for (var i = 0; i < tags.length; i++) {
        var tagTop = document.getElementById(topName[i]);
        tagTop.index=i;
        tagTop.onclick = function () {
            for (var n = 0; n < tags.length; n++) {
                tags[n].className = "";
                cons[n].style.display = "none";
            }
            tags[this.index+1].className = "cur_a";
            cons[this.index+1].style.display = "block";
        }
...
// html5--canvas
var canvasexcel = document.getElementById("excel").getContext("2d");
...
var canhtml5 = function () {
    ...
    var html5T = function (deg) {
        ...
        canvasexcel.clearRect(0, 0, canwidth, canwidth);
        canvasexcel.beginPath();
        canvasexcel.strokeStyle = "#75B143";
        canvasexcel.lineWidth = 10;
        canvasexcel.arc(canwidth / 2, canwidth / 2, radius, 0 - 1 / 2 * Math.PI, r - 1 / 2 * Math.PI, false); //
        canvasexcel.stroke();
    };
